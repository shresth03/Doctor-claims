...
export function OverviewPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-8">
      ...
      {/* The thing a doctor actually came here to do goes first — analytics is a below-the-fold
          concern for someone with seconds between patients, not the first thing they should scan. */}
      ...
      <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-3">
        <KpiTile
          ...
          breachLabel={matchRate.breach === "breach" ? "Below 80%" : "Within range"}
          history={matchRate.history}
          footnote={`${kpi.doctorEditRateSampleSize} codes sampled`}
        />
      ...
      <div className="flex w-full items-start justify-between">
        <span
          className={
            ...
            (tone === "danger" ? "bg-[var(--color-danger-dim)] text-[var(--color-danger)]" : tone === "warning" ? "bg-[var(--color-warning-dim)] text-[var(--color-warning)]" : "bg-black/[0.05] text-[var(--color-text-secondary)]")
          }
        >
          <Icon size={16} />
        </span>
